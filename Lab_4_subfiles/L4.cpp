...
bool push_back(subforwardlist** sfl, int d) {
    subforwardlist* help = new subforwardlist;
    help->data = d;
    help->next = nullptr;
    if (*sfl == nullptr) {
        *sfl = help;
...
bool push_where(subforwardlist** sfl, unsigned int where, int d) {
    if (where == 0) {
        ...
        help->data = d;
        subforwardlist* a = *sfl;
        for (unsigned int i = 1; i < where && a != nullptr; i++) {
            a = a->next;
        }
        if (a == nullptr) {
            return false;
...
int erase_where(subforwardlist** sfl, unsigned int where) {
    if (*sfl == nullptr) {
        ...
        subforwardlist* a = *sfl;
        for (unsigned int i = 1; i < where && a->next != nullptr; i++) {
            a= a->next;
        }
        if (a->next == nullptr) {
            return 0;
